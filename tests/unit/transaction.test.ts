...
type TransactionType = 'bet' | 'win' | 'deposit' | 'withdrawal' | 'adjust' | 'settlement';
...
class TransactionService {
  ...
  creditIn(playerId: string, amount: number, machineId?: string): Transaction {
  ...
  creditOut(playerId: string, amount: number, machineId?: string): Transaction {
    return this.createTransaction({
      type: 'withdrawal',
      amount: -Math.abs(amount),
      playerId,
  ...
  // 下注 (Bet)
  placeBet(playerId: string, amount: number, machineId: string): Transaction {
  ...
  // 派彩 (Win/Settlement)
  settleWin(playerId: string, amount: number, machineId: string): Transaction {
    return this.createTransaction({
      type: 'win',
      amount: Math.abs(amount),
      playerId,
