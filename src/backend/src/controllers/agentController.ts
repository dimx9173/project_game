...
export const getAgents = (req: AuthRequest, res: Response<ApiResponse<PaginatedResponse<Agent>>>): void => {
  const { page = 1, pageSize = 20, parentAgentId, status } = req.query;
  let items = Array.from(agents.values());
  if (parentAgentId) items = items.filter(a => a.parentAgentId === parentAgentId);
...
export const getAgentById = (req: AuthRequest, res: Response<ApiResponse<Agent>>): void => {
  const agent = agents.get(req.params.id);
  agent ? res.json({ success: true, data: agent }) : res.status(404).json({ success: false, error: 'Agent not found' });
};

export const createAgent = (req: AuthRequest, res: Response<ApiResponse<Agent>>): void => {
  const agent: Agent = { id: `agent_${Date.now()}`, ...req.body, createdAt: new Date() };
  agents.set(agent.id, agent);
  res.status(201).json({ success: true, data: agent });
};

export const updateAgent = (req: AuthRequest, res: Response<ApiResponse<Agent>>): void => {
  const agent = agents.get(req.params.id);
