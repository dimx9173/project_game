...
export function Dashboard() {
    const [stats, setStats] = useState({ totalMachines: 0, online: 0, offline: 0, error: 0, totalBet: 0, totalWin: 0, netProfit: 0 });
    ...
    const loadData = async () => {
        try {
            ...
            const [statsRes, machinesRes, alertsRes] = await Promise.all([
                api.request('/monitor/dashboard').catch(() => ({ success: false })),
                api.request('/monitor/machines').catch(() => ({ success: false })),
                api.request('/monitor/alerts').catch(() => ({ success: false }))
            ]);
            if (statsRes.success)
    ...
    return (<div className="dashboard">
      ...
      {alerts.length > 0 && (<div className="alerts-section">
          ...
          <div className="alerts-list">
            {alerts.map((alert, i) => (<div key={i} className={`alert-item alert-${alert.type}`}>
      ...
      {/* Real-time Machines */}
      {machines.length > 0 && (<div className="machines-section">
          <h2>即時機台狀態</h2>
          <div className="machines-grid">
            {machines.map(m => (<div key={m.machineId} className={`machine-card status-${m.status}`}>
