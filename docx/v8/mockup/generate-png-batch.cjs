...
async function main() {
    ...
    for (const file of centralFiles) {
        ...
        const outputFile = path.join(centralDir, file.replace('.html', '.png'));
    ...
    for (const file of localFiles) {
        ...
        const outputFile = path.join(localDir, file.replace('.html', '.png'));
    ...
    console.log(`已存在: ${results.skipped.length} 個`);
